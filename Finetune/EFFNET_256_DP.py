...
net = tf.keras.Model(inputs = inputs, 
                     outputs = predictions, 
                     name=f'effnet.imagenet.{imagesize[0]}.ctl{epoch_toptrain}.ft{total_epochs}.dp{noise_multiplier}')
...
checkpoint_filepath = f'checkpoints/effnet.imagenet.{imagesize[0]}.ctl{epoch_toptrain}.ft{total_epochs}.dp{noise_multiplier}.stage1.hdf5'
...
logdir = f"logs/scalars/effnet{imagesize[0]}ftdpstage1"
...
checkpoint_filepath = f'checkpoints/effnet.imagenet.{imagesize[0]}.ctl{epoch_toptrain}.ft{total_epochs}.dp{noise_multiplier}.stage2.hdf5'
model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
    filepath = checkpoint_filepath,
    save_weights_only = True,
    monitor='val_accuracy',
...
logdir = f"logs/scalars/effnet{imagesize[0]}ftdpstage2"
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=logdir)
